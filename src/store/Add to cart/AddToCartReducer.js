...
export const addToCartreducer = (state = initialState, action) => {
  switch (action.type) {
    case ADD_TO_CART: {
      ...
      const index = cartsArr.findIndex((el) => el._id === action.payload._id);
      if (index === -1) {
    ...
    case INCREASE_CART_ITEMS: {
      ...
      const index = arrCarts.findIndex((el) => el._id === action.payload);
      if (index !== -1) {
    ...
    case DELETE_CART_ITEM: {
      const cartsArr = [...state.carts];
      const index = cartsArr.findIndex((el) => el._id === action.payload);
      if (index !== -1) {
